...
// ─── Attachment Rate (admin/tester-only tab, fetched separately from the main
// dashboard bundle — see /api/attachment-rate) ────────────────────────────────
// "Attached" = a check with this Entree also has a Side/Sweet/Drink item on the
// same check (check-level; can't be pinned to a specific Entree when a check has
// more than one) and/or a "Make it a Meal" modifier tied to this EXACT Entree
// line (item-level exact, via fact_modifiers.parent_selection).
export interface AttachmentOverallRow {
  main_item:            string;
  total_checks:         number;
  checks_with_item:     number;
  checks_with_mod:      number;
  totals:               number;   // deduplicated union: item OR mod
  attachment_rate:      number;   // totals / total_checks, 0-1
  avg_check_attached:    number | null;
  avg_check_unattached:  number | null;
  uplift_per_check:      number | null; // avg_check_attached - avg_check_unattached
  missed_opportunity:    number | null; // (total_checks - totals) * uplift_per_check
}

export interface AttachmentLocationRow {
  main_item:        string;
  location:         string;
  total_checks:     number;
  checks_with_item: number;
...
export interface AttachmentChannelRow {
  main_item:        string;
  channel:          string;   // IN_HOUSE | TPD | APP
  total_checks:     number;
  checks_with_item: number;
  checks_with_mod:  number;
  totals:           number;
...
export interface AttachmentCompanionPair {
  main_item: string;
  checks:    number;
}

export interface AttachmentCompanionRow {
  companion_name:      string;
  companion_category:  string;   // Sides | Sweets | NA Drinks | Alc Drinks | Make it a Meal
  total_attach_checks: number;
  pairs:               AttachmentCompanionPair[]; // every Entree it pairs with, sorted desc by checks
  pairing_precision:   'exact' | 'approx'; // exact = modifier link; approx = check-level only
}

export interface AttachmentData {
  overall:   AttachmentOverallRow[];
  byLocation: AttachmentLocationRow[];
  byChannel:  AttachmentChannelRow[];
  companions: AttachmentCompanionRow[];
}
